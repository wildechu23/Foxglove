...
public:
    ...
    static std::vector<PushConstantInfo> reflect_push_constants(Shader* shader) {
        std::span<uint32_t> code = shader->m_code;
        ...
        uint32_t count = 0;
        spvReflectEnumeratePushConstantBlocks(&spvmodule, &count, nullptr);
        std::cout << "push count: " << count << std::endl;
        if (count == 0) return {};

        std::vector<PushConstantInfo> infos;

        std::vector<SpvReflectBlockVariable*> blocks(count);
        spvReflectEnumeratePushConstantBlocks(&spvmodule, &count, blocks.data());
